<template>
  <div class="row flex-column d-flex align-items-center mt-1">
    <h1 class="col-md-6 col-lg-4 col-sm-12 mb-4 text-black">
      {{ t("auth.sign_up.confirm_and_signup") }}
    </h1>
    <p
      class="mb-4 connected-info-google col-md-6 col-lg-4 col-sm-12"
      v-if="userInfo"
    >
      Your <span>Google</span> account
      <span>{{ userInfo.attributes.name }}</span> will be connected to your new
      Sharebus account.
    </p>
    ...
    <form @submit.prevent="onSubmit" class="col-md-6 col-lg-4 col-sm-12">
      <div class="form-input-wrapper col-md-12 col-sm-12 col-lg-12">
        ...
        <BaseInput
          :readonly="true"
          input-group-class="signup-input readonly-email-bg"
          :label="t('auth.common.email')"
          v-model="values.username"
          type="text"
          :modifier-class="`${
            usernameErrMsg ? 'is-invalid' : ''
          }  auth-input-height text-start readonly-mode`"
      ...
      <div class="col-md-12 col-sm-12 col-lg-12 mt-3">
        <BaseButton
          type="submit"
          button-class=" rounded-pill sb-btn-primary sb-btn-lg fw-bold w-100"
      ...
      <div class="col-md-12 col-sm-12 col-lg-12 mt-3">
        <BaseButton
          button-class="sb-btn-danger sb-btn-lg fw-bold px-5"
          @click="handleCancel"
          :button-text="t('auth.sign_up.cancel')"
        >
        </BaseButton>
      </div>
    </form>
 ...
 */
const { handleSubmit, values, setValues, meta, submitCount } = useForm({
  ...
  initialValues: {
    username: storeUser.getUserInfo["attributes"].email,
    attributes: {
      phone_number: "",
      name: storeUser.getUserInfo["attributes"].name,
    },
  },
});
...
const onSubmit = handleSubmit((values) => {
  ...
  storeSignUp.handleSignUpInfo(values, true);
  loader.changeLoadingStatus({ isLoading: true });
  if (typeof userInfo.value.attributes.phone_number === "undefined") {
    updateUser(values);
  } else if (
    userInfo.value.attributes.phone_number !== values.attributes.phone_number
  ) {
    updateUser(values);
  } else {
    storeUser.handleVerifyUserAttribute("phone_number").then(() => {
...
const updateUser = (values) => {
  storeUser
    .handleUpdateUser(values.attributes)
    .then((result) => {
